...
export const extractHostname = (url?: string): string => {
  ...
  return (parsedUrl && parsedUrl.hostname!) || (parsedUrl && parsedUrl.host!)
}
...
const isUrlBlacklisted = (host, path) => {
  ...
  const urlBlacklistMap: any = {
    ...
    '127.0.0.1': (url, pattern, urlPath) =>
      url === pattern && urlPath.startsWith('/2018-06-01/runtime/invocation/'),
...
const isUrlIgnoredByUser = (host) => {
  const ignorePatterns: string[] =
    (process.env.RECAP_DEV_IGNORE_HTTP_PATTERNS &&
      process.env.RECAP_DEV_IGNORE_HTTP_PATTERNS.split(',')) ||
    []
