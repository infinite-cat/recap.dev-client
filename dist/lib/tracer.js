...
var simple_trace_store_1 = require("./services/simple-trace-store");
var utils_1 = require("./module-trackers/utils");
...
var Tracer = /** @class */ (function () {
    function Tracer() {
        this.traceStore = new simple_trace_store_1.SimpleTraceStore();
    }
    /**
     * Get a current trace
    ...
    Tracer.prototype.setTraceError = function (error) {
        var trace = this.traceStore.getCurrentTrace();
        if (trace) {
            trace.error = utils_1.serializeError(error);
            trace.status = 'ERROR';
        }
    };
    /**
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        dataBuffer = zlib_1.gzipSync(Buffer.from(JSON.stringify(traceToSync), 'utf-8'));
                        if (!config_1.config.syncEndpoint) {
                            log_1.debugLog('RECAP_DEV_SYNC_ENDPOINT env variable is empty, skipping sync');
                            return [2 /*return*/];
                        }
                        return [4 /*yield*/, node_fetch_1.default(config_1.config.syncEndpoint, {
                                method: 'POST',
                                body: dataBuffer,
                                headers: {
                                ...
                                timeout: config_1.config.syncTimeout,
                            })];
                    case 1:
                        _a.sent();
                        log_1.debugLog('recap.dev sync took: ', Date.now() - timestamp, ' ms');
                        return [3 /*break*/, 3];
                    case 2:
                        e_1 = _a.sent();
                        log_1.debugLog('Warning: error when syncing trace data');
                        log_1.debugLog(e_1);
                        return [3 /*break*/, 3];
